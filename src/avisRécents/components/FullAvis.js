  ...
  return (
    <Card sx={{ 
      ...
      <CardContent sx={{ paddingTop: 1, flexGrow: 1 }}>
        ...
        {/* Affichage du bouton seulement si nécessaire */}
        ...
        <Box sx={styles.modal}>
          ...
          {avisData && avisData.user ? (
            <Box sx={styles.userSection}>
              <Avatar src={avisData.user.thumbnail} alt={avisData.user.name} sx={styles.avatar} />
              <Box>
                <Typography fontWeight="bold">{avisData.user.name}</Typography>
                <Typography variant="body2" color="text.secondary">
                  {avisData.date} • {avisData.reviewCount} avis
                </Typography>
              </Box>
            </Box>
          ) : (
          ...
          <Typography sx={styles.commentText}>{avisData?.text}</Typography>
...
FullAvis.propTypes = {
  avisData: PropTypes.shape({
    ...
    text: PropTypes.string.isRequired, // Ajouté pour être sûr qu'on a toujours du texte
    link: PropTypes.string,
